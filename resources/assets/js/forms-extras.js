...
$(function () {
  ...
  if (id != '') {
    $.ajax({
      ...
      dataType: 'json',
      success: function (res) {
        let contacts = res.data.contacts_ao;
        contacts = contacts.split(',');
        let names = res.data.contacts_nom;
        names = names.split(',');
        let prenoms = res.data.contacts_prenom;
        prenoms = prenoms.split(',');
        let postes = res.data.contacts_poste;
        postes = postes.split(',');
        let mails = res.data.contacts_mail;
        mails = mails.split(',');
        let gsms = res.data.contacts_gsm;
        gsms = gsms.split(',');
        let idClients = res.data.contacts_idClient;
        idClients = idClients.split(',');

        $.each(contacts, function (i, item) {
          var clone = content.clone();
          clone.find('input[name="nom"]').val(names[i]);
          clone.find('input[name="prenom"]').val(prenoms[i]);
          clone.find('input[name="poste"]').val(postes[i]);
          clone.find('input[name="mail"]').val(mails[i]);
          clone.find('input[name="gsm"]').val(gsms[i]);
          clone.find('input[name="idClient"]').val(idClients[i]);
          clone.find('input').each(function () {
            var name = $(this).attr('name');
