...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('models.file.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.files.update", [$file->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('size') ? 'is-invalid' : '' }}" type="number" name="size" id="size" value="{{ old('size', $file->size) }}" step="0.01">
                @if($errors->has('size'))
                    <div class="invalid-feedback">
                        {{ $errors->first('size') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('mime_type') ? 'is-invalid' : '' }}" type="text" name="mime_type" id="mime_type" value="{{ old('mime_type', $file->mime_type) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ocr_content') ? 'is-invalid' : '' }}" type="text" name="ocr_content" id="ocr_content" value="{{ old('ocr_content', $file->ocr_content) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $file->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('commentaire') ? 'is-invalid' : '' }}" name="commentaire" id="commentaire">{{ old('commentaire', $file->commentaire) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('associated_to') ? 'is-invalid' : '' }}" type="text" name="associated_to" id="associated_to" value="{{ old('associated_to', $file->associated_to) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('site') ? 'is-invalid' : '' }}" name="site_id" id="site_id" required>
                    @foreach($sites as $id => $entry)
                        <option value="{{ $id }}" {{ (old('site_id') ? old('site_id') : $file->site->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('site'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('parent') ? 'is-invalid' : '' }}" name="parent_id" id="parent_id">
                    @foreach($parents as $id => $entry)
                        <option value="{{ $id }}" {{ (old('parent_id') ? old('parent_id') : $file->parent->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('archived') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="archived" id="archived" value="1" {{ $file->archived || old('archived', 0) === 1 ? 'checked' : '' }}>
