...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('models.consommation.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.consommations.update", [$consommation->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="date">{{ trans('models.consommation.fields.date') }}</label>
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $consommation->date) }}">
                @if($errors->has('date'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('valeur') ? 'is-invalid' : '' }}" type="number" name="valeur" id="valeur" value="{{ old('valeur', $consommation->valeur) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    ...
                    @foreach(App\Models\Consommation::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $consommation->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('batiment') ? 'is-invalid' : '' }}" name="batiment_id" id="batiment_id" required>
                    @foreach($batiments as $id => $entry)
                        <option value="{{ $id }}" {{ (old('batiment_id') ? old('batiment_id') : $consommation->batiment->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
