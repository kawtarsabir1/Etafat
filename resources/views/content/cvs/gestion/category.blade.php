...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function(data) {
                if ($.isEmptyObject(data.error)) {
                    alert('Category ajouté avec succés.');
                    location.reload();
                } else {
    ...
    function deleteCategory(id) {
        if (confirm("Voulez-vous vraiment supprimer ce Category ?")) {
            $.ajax({
                url: "/cv/deletecategory/" + id,
                type: 'DELETE',
                success: function(data) {
                    if ($.isEmptyObject(data.error)) {
                        alert('Category supprimé avec succés.');
                        location.reload();
                    } else {
                        printErrorMsg(data.error);
                    }
...
<h4 class="fw-bold py-3 mb-4">
    <span class="text-muted fw-light">CVtéque /</span> Gestion Des categories
</h4>
                ...
                <div>
                    ...
                    <div>
                        <ul class="list-group mb-4">
                            ...
                            <li class="list-group-item">{{ $category->categoryNom }} <button type="button" onclick="deleteCategory({{$category->id}})" class="btn btn-danger btn-sm float-end">Supprimer</button></li>
                            @endforeach
                        </ul>
                    </div>
                </div>
