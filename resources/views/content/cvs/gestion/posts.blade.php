...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function(data) {
                if ($.isEmptyObject(data.error)) {
                    alert('Poste ajouté avec succés.');
                    location.reload();
                } else {
    ...
    function deletePost(id) {
        if (confirm("Voulez-vous vraiment supprimer ce poste ?")) {
            $.ajax({
                url: "/cv/deletepost/" + id,
                type: 'DELETE',
                success: function(data) {
                    if ($.isEmptyObject(data.error)) {
                        alert('Poste supprimé avec succés.');
                        location.reload();
                    } else {
                        printErrorMsg(data.error);
                    }
    ...
    $(function () {
        ...
        $('.btn-update').click(function (e) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if ($.isEmptyObject(data.error)) {
                        alert('Poste modifié avec succés.');
                        location.reload();
                    } else {
                        printErrorMsg(data.error);
                    }
...
<h4 class="fw-bold py-3 mb-4">
    <span class="text-muted fw-light">CVtéque /</span> Gestion Des Postes
</h4>
            ...
            <form id="create-form" class="formInfos">
                <div>
                    ...
                    <div class="col-12 row mb-4">
                        <div class="col-md-6">
                            <label class="form-label" for="Nom">Nom du Poste</label>
                    ...
                    <div>
                        <ul class="list-group mb-4">
                            @php
                                $sortedPosts = $posts->sortBy('id');
                            @endphp
                            @foreach($sortedPosts as $post)
                            <li class="list-group-item">
                                <p id="text-{{$post->id}}">{{ $post->postNom }}</p>
                                <button type="button" onclick="deletePost({{$post->id}})" class="btn btn-danger btn-sm float-end">Supprimer</button>
                                <button type="button" onclick="editPoste({{$post->id}})" class="btn btn-primary btn-sm float-end me-2">Modifier</button>
                            </li>
                            @endforeach
                        </ul>
                    </div>
