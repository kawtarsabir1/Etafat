...
<script type="text/javascript">
    ...
    function deletePay(id) {
        if (confirm("Voulez-vous vraiment supprimer ce Pay ?")) {
            $.ajax({
                url: baseUrl + 'ao/champ/pay/' + id,
                type: 'DELETE',
                success: function(data) {
                    if ($.isEmptyObject(data.error)) {
                        alert('Pay supprimé avec succés.');
                        location.reload();
                    } else {
                        printErrorMsg(data.error);
                    }
...
<h4 class="fw-bold py-3 mb-4">
    <span class="text-muted fw-light">CVtéque /</span> Gestion Des Pays
</h4>
    ...
    <div id="wizard-checkout" class="bs-stepper wizard-icons wizard-icons-example mt-2">
        <div class="bs-stepper-content">
            <form id="create-form" class="formInfos">
                <div>
                    <h5>Ajouter de Pay</h5>
                    <div class="col-12 row mb-4">
                        <div class="col-md-6">
                            <label class="form-label" for="Nom">Nom de Pay</label>
                    ...
                    <div>
                        <ul class="list-group mb-4">
                            @foreach($pays as $pay)
                            <li class="list-group-item">{{ $pay->pay }} <button type="button" onclick="deletePay({{$pay->id}})" class="btn btn-danger btn-sm float-end">Supprimer</button></li>
                            @endforeach
                        </ul>
                    </div>
                </div>
