...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function(data) {
                if ($.isEmptyObject(data.error)) {
                    alert('Partenaire ajouté avec succés.');
                    location.reload();
                } else {
    ...
    function deletePartenaire(id) {
        if (confirm("Voulez-vous vraiment supprimer ce Partenaire ?")) {
            $.ajax({
                url: baseUrl + 'ao/champ/partenaire/' + id,
                type: 'DELETE',
                success: function(data) {
                    if ($.isEmptyObject(data.error)) {
                        alert('Partenaire supprimé avec succés.');
                        location.reload();
                    } else {
                        printErrorMsg(data.error);
                    }
...
<h4 class="fw-bold py-3 mb-4">
    <span class="text-muted fw-light">CVtéque /</span> Gestion Des Partenaires
</h4>
